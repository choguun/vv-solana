...
import type { BlockUpdate, Coords3 } from "@vv-libs/core";

import topStarredRepos from "../assets/data/topStars.json";
import { ColorText } from "../components/ColorText";
import { grayReplacement, twitterLink } from "../constants";
...
export function Chat() {
  ...
  useLayoutEffect(() => {
    ...
    // chat.addCommand('hub', () => {
    //   reloadAndJoinWorld('main');
    // });

    // chat.addCommand('lobby', () => {
    //   reloadAndJoinWorld('main');
    // });

    // chat.addCommand('lab', () => {
    //   reloadAndJoinWorld('lab');
    // });

    // chat.addCommand("discord", () => {
    ...
    if (isAdmin()) {
      ...
      chat.addCommand("wallpaper", (rest) => {
        const target = voxelInteract.target;
        if (!target) return;

        const imageSrc = rest.trim();
        if (!imageSrc || !imageSrc.startsWith("http")) return;

        world.setBlockEntityDataAt(...target, {
          type: "wallpaper",
          resolutionFactor: 40,
    ...
    const handleDisconnect = () => {
      chat.onChat({
        type: "system",
        body: `$#C40C0C$Disconnected! Tell $${twitterLink}$@Admin $#C40C0C$to restart the server...`,
      });
    };
